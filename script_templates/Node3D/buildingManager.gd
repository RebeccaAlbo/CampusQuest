...
func add_collision_to_mesh(mesh_instance: MeshInstance3D):
	...
	if mesh_instance.mesh:
		if mesh_instance.name.to_lower() == "map_20_osm_paths_cycleway" or mesh_instance.name.to_lower() == "map_15_osm_forest" or mesh_instance.name.to_lower() == "map_17_osm_areas_pedestrian" or mesh_instance.name.to_lower() == "map_15_osm_vegetation" or mesh_instance.name.to_lower() == "map_15_osm_water":
			return
